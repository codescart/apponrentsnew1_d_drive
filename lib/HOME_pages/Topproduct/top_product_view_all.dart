...
class top_product_view_all extends StatefulWidget {
  const top_product_view_all({Key? key}) : super(key: key);

  @override
  State<top_product_view_all> createState() => _top_product_view_allState();
}

class _top_product_view_allState extends State<top_product_view_all> {
  ...
  mobile(BuildContext context) {
    return FutureBuilder<List<top_product>>(
        future: Allproduct(),
...
Future<List<top_product>> Allproduct() async {
  final prefs = await SharedPreferences.getInstance();
  final key = 'deviceId';
  final deviceId = prefs.getString(key) ?? "0";
  final response = await http.post(
    Uri.parse(
        Apiconst.All_Products+"?device_id=$deviceId&count=100&type=2"),
  ...
  print(response.body);
  List<top_product> alllround = [];
  for (var o in jsond) {
    top_product al = top_product(
      o["id"],
      o["product_image"],
      o["other_images"],
      o["category"],
